...
export default function ChildrenCard({ id, name, photo }: ChildrenCardProps) {
  return (
    <Link href={`/children/${id}`} className="block group relative bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-200 aspect-[3/4] w-full max-w-[200px] mx-auto cursor-pointer">
      {/* Photo as Background */}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 z-10 transition-opacity duration-300 group-hover:opacity-0">
        <h3 className="text-lg font-semibold text-background text-center drop-shadow-lg">
          {name}
        </h3>
      </div>
      ...
      <div className="absolute inset-0 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 px-4">
        <p className="text-base font-medium text-background text-center drop-shadow-lg">
